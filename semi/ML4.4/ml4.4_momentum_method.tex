...
\]
ここで、$\mathbb{I}$は単位行列(\textbf{Unitary Matrix"})行列 $\mathbb{I}-M$ が正則（逆行列が存在）であると仮定すると、
\[ S_n = (\mathbb{I} - M^n)(\mathbb{I} - M)^{-1} \]
したがって、状態 $s_n$ の一般項は以下のように表される。
\begin{empheq}{equation}
    s_n = M^n s_0 + (\footnotemark[3]\mathbb{I}_2 - M^n)(\mathbb{I}_2 - M)^{-1} \beta \label{eq:closed_form}
\end{empheq}
\footnotetext[3]{$\mathbb{I}_2$ とは$2\times 2$の単位行列である。}
...
\]
この2次方程式の解 $\lambda_1, \lambda_2$ について分析する（\textbf{7ページの図2参照}）
\begin{enumerate}
...
\begin{align*}
    s^* = \lim_{n \to \infty} s_n &= (I_2 - O)(I_2 - M)^{-1} \beta \\
    &= (I_2 - M)^{-1} \beta
\end{align*}
