...
を得る。したがって
\[ \lambda_0 = \frac{1}{2}\|\nabla f(x^0)\|,\, \lambda_1 = -\frac{1}{2}\|\nabla f(x^0)\|\]
最小値を与えるのは$\lambda_0$の場合であり, その$\lambda_0$を$\nu$に代入すると
\[ \nu^* = -\frac{\nabla f(x^0)}{\|\nabla f(x^0)\|} \]
となる。
\end{proof}
\noindent
...
\[
 f(x^0+\eta \nu^*) - f(x^0)
   =\eta \langle \nabla f(x^0), \nu^*\rangle\quad\footnotemark[2] = -\eta \dfrac{\langle \nabla f(x^0),\nabla f(x^0)}{\|\nabla f(x^0)\|} = -\eta \|\nabla f(x^0)\|
\]
...
\begin{exm}{\ruby{峡谷}{きょうこく}関数の例}
  ...
  \item \(0<\delta<1\) のとき，$1-\delta \in (0,1)\implies |1 -\delta|<1\implies \displaystyle\lim_{n\to\infty} (1-\delta)^n = 0$。従って、 $(y^n)_n$は収束する。 $x^{n+1}-x^n=\delta\implies$ $(x^n)_n$ は公差 \(\delta\)の
