...
動画の予測変数 $\hat{z_t}$ が直前のフレーム $\hat{  }$ のみに依存して生成される場合、各ステップで生じる微小な推論誤差が時間とともに累積し、非線形な系において指数関数的に増幅される現象を指す。
\end{dfn}
...
\begin{tikzpicture}[>=stealth, scale=0.9, transform shape]
    ...
    \begin{scope}[xshift=0cm]
        ...
        % 結論のテキスト
        \node[draw, fill=white, rounded corners, align=center, red!80!black] at (0, 0) {\textbf{完全に潰される} \\ 増幅率（固有値） $\approx 0$};
    \end{scope}
    ...
    \begin{scope}[xshift=9cm]
        ...
        \node[align=center, blue!80!black] at (0, 2.9) {（制約を満たす / 沿軌）};
        ...
        \node[green!50!black] at (-1.5, 0.3) {接空間 $\mathcal{T}_z\mathcal{M}$};
        ...
        % 結論のテキスト（平面上をそのままスライドするだけ）
        \node[draw, fill=white, rounded corners, align=center, blue!80!black] at (1.5, -1.2) {\textbf{そのまま保存される} \\ 増幅率（固有値） $\approx 1$};
    \end{scope}

\end{tikzpicture}
\caption{多様体から逸脱する誤差（方向A）は射影によって消去され、多様体に沿う誤差（方向B）のみが等倍で保存されるため、最大増幅率は1となる。}
\end{figure}
...
よって、十分大きな$n$に対して $\displaystyle\sup_{\|x\| = 1} \|\mathbf{J}^n \mathbf{x}\| = \|\mathbf{J}^n\| \approx |\lambda_m|^n =\rho(\mathbf{J})^n$ が示された。
\end{proof}
