...
確率変数(Random Variance)$W$の値を$x_j-x_k$,\ 確率変数(R.V.)$X$の値を$x-x_k$,\ 確率変数(R.V.)$Y$の変数を$x_j-x$,\ $W$の密度関数を$f_{W}(w)$とすると、$W=((x-x_k)+(x_j-x))=X+Y$となる。したがって、
$$
...
$$
ここで、$z=x+y$とおくと、$-\infty< z-y\leq w-y\ \implies\ -\infty < z\leq w$となる。一方で、$\dfrac{dx}{dz}=1$である。以下のように書ける：
$$
\int_{-\infty}^{\infty} f_{Y}(y)\left(\int_{-\infty}^{w-y} f_{X}(x)dx\right)dy=\int_{-\infty}^{\infty} f_{Y}(y)\left(\int_{-\infty}^{w} f_{X}(z-y)dz\right)dy
$$
\noindent
したがって、
\begin{align*}
  f_{W}(w)=\dfrac{d}{dw}F_{W}(w)&=\int_{-\infty}^{\infty} f_{Y}(y)\ \left[\dfrac{d}{dw}\int_{-\infty}^{w} f_{X}(z-y)dz\right]dy  \\
